...
1. `app/runtime.py`
   Assembles repository, parser, retrieval, report, analysis, installer, and orchestrator services without FastAPI.
2. `app/desktop/bridge.py`
   Exposes desktop-safe bridge methods such as `fetch_health`, `upload_documents`, `run_retrieval`, and `run_orchestrator`.
3. `frontend/`
   Vite/React shell that detects `window.pywebview` and calls the bridge directly instead of `fetch`.
...
The Inno Setup installer is defined in [`installer/TeamMindHub.iss`](./installer/TeamMindHub.iss).
...
More variables are listed in [`.env.example`](./.env.example).
...
GitHub release automation template is provided in [`.github/workflows/release-desktop.yml`](./.github/workflows/release-desktop.yml). It builds the Windows installer on `windows-latest`, uploads the installer artifact, and attaches it to GitHub Releases when triggered by a published release.
